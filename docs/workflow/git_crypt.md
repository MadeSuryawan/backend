...
```mermaid
flowchart LR
    %% Contrast improvements for dark/light mode environments
    classDef plain fill:#f8fafc,stroke:#94a3b8,stroke-width:2px,color:#0f172a
    classDef encrypted fill:#ecfdf5,stroke:#10b981,stroke-width:2px,color:#0f172a

    Editor["Your Editor<br>(Plaintext)"]:::plain -->|Git Commit| Commit["Git Commit<br>(Encrypted)"]:::encrypted
    Commit -->|git push| Remote["GitHub/GitLab<br>(Encrypted)"]:::encrypted
    Remote -->|git pull / checkout| Editor

    linkStyle default stroke:#3b82f6,stroke-width:3px,color:#0f172a
```
...
### Removing Access (Key Rotation)

Because `git-crypt` does not have an automatic `rotate-key` command, revoking access requires manually re-initializing the encryption environment.

```bash
# 1. Ensure all secrets are decrypted
git-crypt unlock

# 2. De-configure and remove old keys
rm -rf .git-crypt
git commit -am "Security: Remove compromised/old git-crypt keys"

# 3. Re-initialize symmetric key
git-crypt init
git-crypt export-key ~/git-crypt-backend-new.key

# 4. Re-add files to encrypt with the new key
git add secrets/
git commit -m "Security: Re-encrypt secrets with new key"

# 5. Add authorized team members' GPG keys again
git-crypt add-gpg-user user@email.com

# 6. Push changes and distribute new key
git push
```
...
```bash
# 1. Unlock repo and remove old configuration completely
git-crypt unlock
rm -rf .git-crypt
git commit -am "Security: Remove compromised git-crypt configuration"

# 2. Re-initialize and encrypt with a new key
git-crypt init
git add secrets/
git commit -m "Security: Re-encrypt secrets with new key"

# 3. Export and distribute the new key
git-crypt export-key ~/git-crypt-backend-new.key
# Securely share with team over encrypted channels

# 4. Push updates to remote
git push

# 5. Revoke old key everywhere
...
```bash
# 1. Immediately rotate key (decrypt, rm .git-crypt, re-init, re-encrypt)
# (See Scenario 3: Compromised Key above for exact command flow)
...
# Team
git-crypt add-gpg-user user@email.com
# Note: Key rotation requires re-initialization (no 'rotate-key' command)
